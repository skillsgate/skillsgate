	...
	useEffect(() => {
		...
		async function fetchData() {
			const [skillsRes, reposRes] = await Promise.all([
				api.get<PublisherSkillsData>("/api/dashboard/publisher/skills"),
				api.get<{ repos: ConnectedRepo[] }>("/api/connected-repos"),
			]);
		...
		<div>
			...
			<div className="flex items-start justify-between mb-8">
				<div>
					...
					<p className="text-[14px] text-muted mt-1">
						Manage your published skills and connected repos.
					</p>
				</div>
				<Link
			...
			{data.skills.length > 0 ? (
				...
				<div className="rounded-xl border border-border bg-card-bg p-12 text-center">
					...
					<p className="text-[13px] text-muted/70 mb-4">
						Upload your first skill or connect a GitHub repo.
					</p>
					<div className="flex items-center justify-center gap-3">
						...
						<Link
							...
							className="inline-flex px-4 py-2 text-[13px] font-medium text-muted border border-border rounded-lg hover:text-foreground hover:border-foreground/20 transition-colors"
						>
							Connect Repo
						</Link>
					</div>
			...
			<section className="mt-10">
				...
				{reposLoading ? (
					...
					<div className="rounded-xl border border-border/50 bg-card-bg/50 p-8 text-center">
